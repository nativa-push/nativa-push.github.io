...
    // Comprobar si los productos están ocultos en el HTML
    const productosOcultos = document.querySelectorAll('.productos .producto[style="display: none;"]');
    const submenuOculto = document.querySelector('.submenu li:not(:first-child)');

    // Si hay elementos ocultos, no modificar el HTML.
    // La función principal se encargará de mostrarlos si es necesario.
    if (productosOcultos.length > 0) {
        console.log('Productos ocultos detectados. Dejando el HTML como está.');
        // Asegurarse de que el contenedor de moldes se muestre
        moldesContainer.style.display = 'flex';
    } else {
        // Si no hay elementos ocultos, renderizar la página principal con todos los productos
        mostrarTodosMoldes();
    }
});
